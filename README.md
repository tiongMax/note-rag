...
- **Grounded chat** — duplicate-free context assembly, explicit token budgets,
  persisted per-turn memories, verbatim recent history, semantic older-memory
  recall, source citations, and server-sent-event streaming.
...
| `CHAT_HISTORY_MAX_TOKENS` | `2000` | Maximum assembled conversation-memory tokens before the global prompt budget is applied. |
| `CHAT_MEMORY_RECENT_TURNS` | `4` | Completed turns retained verbatim. |
| `CHAT_MEMORY_SEMANTIC_K` | `2` | Maximum older embedded summaries recalled per question. |
| `CHAT_MEMORY_SEMANTIC_MIN_SIMILARITY` | `0.25` | Minimum cosine similarity for older-memory recall. |
| `CHAT_MEMORY_SUMMARY_MAX_TOKENS` | `64` | Maximum deterministic extractive summary size per completed turn. |
...
Every successfully validated user/assistant pair is committed atomically with
a bounded extractive summary and an embedding generated by the configured
embedding provider. For a follow-up, the newest four complete turns stay
verbatim and up to two older summaries are selected by cosine similarity. The
history, retrieved context, fixed prompt text, and output reserve share one
explicit prompt-token budget. Selected-memory counts and token usage are
returned in the chat response and SSE metadata for auditability.

If memory embedding is temporarily unavailable, the completed turn and its
summary are still persisted, but it is not labeled or used as semantic memory.
The versioned deterministic fact-retention benchmark lives under
`benchmarks/conversation_memory`; it does not claim model-generated answer
accuracy.
...
semantic safety or factuality guarantee. Their synthetic regression evaluation
